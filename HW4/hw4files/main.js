document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleSubmitWord() {
		...
		if (currentWordArray.length !== 5) {
			...
			async function checkGuessAndDetermineColors(guess) {
				console.log("Player's guess: ", guess);
				let myResponse = await fetch("quiz4.php?guess=" + guess);
				let guessColors = await myResponse.json();
				console.log("Result from server: ", guessColors);

				//loop to see if all green = you win
				let win = true;
				for (let i = 0; i <= 4; i++) {
			...
			checkGuessAndDetermineColors(mostRecentGuess);
		}
    }
